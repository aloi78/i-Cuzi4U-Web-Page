...
export default function CarpetCleaningPage({ onBack, onWhatsApp }: CarpetCleaningPageProps) {
  const { t, i18n } = useTranslation();
  const [activeFaq, setActiveFaq] = useState<number | null>(null);

  useEffect(() => {
    // Dynamic SEO updates specifically for the Carpet Page
    const titles: Record<string, string> = {
      en: "Professional Carpet Cleaning & Rug Extraction Klang Valley | i-Cuzi4U",
      ms: "Servis Cuci Karpet Pejabat & Rumah Lembah Klang | i-Cuzi4U",
      zh: "巴生谷专业地毯清洁与商用及家用重度喷抽清洗除螨 | i-Cuzi4U"
    };
    
    const descriptions: Record<string, string> = {
      en: "Industrial-grade carpet cleaning service in Klang Valley. Deep water extraction & sanitization for office carpets, exhibition rugs, and home carpets with eco-friendly solutions.",
      ms: "Servis pembersihan karpet gred industri di Lembah Klang. Cuci, bilas, dan sedut keluar kotoran karpet pejabat, dewan pameran, dan rumah kediaman secara mesra alam.",
      zh: "巴生谷专业温水喷抽地毯清洗。采用德国 Kärcher® 专业喷抽与深层回收技术，深度清除顽固油污、咖啡渍、宠物异味与尘螨，安全环保、快速干燥。"
    };
  ...
  const problems = [
    { id: 'spills', icon: <Sparkles className="text-[#0D49CD]" size={28} /> },
    { id: 'allergens', icon: <AlertTriangle className="text-[#FFD800]" size={28} /> },
    { id: 'odors', icon: <Clock className="text-action" size={28} /> },
    { id: 'wear', icon: <TrendingUp className="text-red-400" size={28} /> }
  ];
  ...
  const sizes = [
    {
      id: 'rug',
      titleKey: 'carpet_page.sizes.rug_title',
      priceKey: 'carpet_page.sizes.rug_price',
      featuresKey: 'carpet_page.sizes.rug_features',
      msg: 'Rug, Residential & SOHO Carpet',
      featured: true
    }
  ];
  ...
  const handleBookClick = (sizeName: string) => {
    const baseMsg = t('carpet_page.cta_msg');
    onWhatsApp(`${baseMsg} ${sizeName}`);
  };

  return (
    <div className="bg-app-bg min-h-screen" id="carpet-cleaning-page">
      {/* Breadcrumb Header */}
      <div className="bg-white border-b border-gray-100 py-4 sticky top-[72px] z-30 shadow-sm">
        <div className="container mx-auto px-4 flex items-center justify-between">
          <button 
            id="carpet-back-button"
            onClick={onBack}
            className="flex items-center gap-2 text-primary font-bold hover:translate-x-[-4px] transition-transform text-sm md:text-base cursor-pointer"
          >
            <ArrowLeft size={18} />
            <span>{t('carpet_page.back_btn')}</span>
          </button>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-7">
              ...
              >
                ...
                <h1 className="text-3xl md:text-5xl font-extrabold text-primary leading-tight mb-6">
                  {t('carpet_page.hero_title')}
                </h1>
                
                <p className="text-lg md:text-xl text-gray-600 mb-8 leading-relaxed max-w-2xl">
                  {t('carpet_page.hero_subtitle')}
                </p>
                ...
                <div className="grid grid-cols-2 gap-4 mb-10 max-w-lg">
                  ...
                  <div className="flex items-center gap-2">
                    <CheckCircle2 size={20} className="text-action" />
                    <span className="font-semibold text-sm text-gray-700">High-Pressure Extraction</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle2 size={20} className="text-action" />
                    <span className="font-semibold text-sm text-gray-700">Office & Event Experts</span>
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row gap-4">
                  <button 
                    onClick={() => handleBookClick('Wall-to-Wall Office Carpet')}
                    className="btn-primary text-lg w-full sm:w-auto cursor-pointer"
                  >
                    <MessageCircle size={22} />
                    <span>{t('carpet_page.cta_book')}</span>
                  </button>
                  <a 
                    href="#pricing"
            ...
            <div className="lg:col-span-5">
              ...
              >
                <img 
                  src="https://lh3.googleusercontent.com/d/1aS8s_ScBV6GMikA0FfwzMtJQ_DCY0G1Q"
                  alt={t('seo.carpet_alt')}
                ...
                <div className="absolute bottom-6 left-6 bg-white/90 backdrop-blur-md px-5 py-3.5 rounded-2xl shadow-lg border border-white/20">
                  ...
                  <p className="text-lg font-extrabold text-primary">Kärcher® Spray-Extraction</p>
                </div>
              </motion.div>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('carpet_page.why_section_title')}
            </h2>
            <p className="text-gray-600 text-base md:text-lg">
              {t('carpet_page.why_section_desc')}
            </p>
          </div>
            ...
            {problems.map((prob, idx) => (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-primary mb-3">
                  {t(`carpet_page.problems.${prob.id}_title`)}
                </h3>
                <p className="text-gray-500 text-sm leading-relaxed">
                  {t(`carpet_page.problems.${prob.id}_desc`)}
                </p>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('carpet_page.process_section_title')}
            </h2>
            <p className="text-gray-600 text-base md:text-lg">
              {t('carpet_page.process_section_desc')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {stages.map((stage, idx) => (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-primary mb-3">
                  {t(`carpet_page.stages.${stage}_title`)}
                </h3>
                <p className="text-gray-600 text-sm leading-relaxed">
                  {t(`carpet_page.stages.${stage}_desc`)}
                </p>
              </motion.div>
            ))}
          </div>
      ...
      <section id="pricing" className="py-20 bg-app-bg border-y border-gray-100">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-12">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('carpet_page.pricing_section_title')}
            </h2>
            <p className="text-gray-600 text-base md:text-lg mb-4">
              {t('carpet_page.pricing_section_desc')}
            </p>
            {/* Coming Soon Notice Badge */}
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#FFD800]/20 text-primary border border-[#FFD800] rounded-full text-xs font-black tracking-wide uppercase">
              <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
              Shop Wash / Takeaway Service Coming Soon!
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-100 rounded-[2rem] p-6 md:p-8 max-w-4xl mx-auto mb-12 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex items-center gap-4 justify-center md:justify-start">
              ...
              <div>
                <h4 className="font-extrabold text-primary text-base leading-tight">
                  {t('carpet_page.highlights.effective_title')}
                </h4>
                <p className="text-xs text-gray-500 font-medium mt-1">
                  {t('carpet_page.highlights.effective_sub')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 justify-center md:justify-start border-t md:border-t-0 md:border-x border-gray-100 pt-6 md:pt-0 md:px-8">
              ...
              <div>
                <h4 className="font-extrabold text-primary text-base leading-tight">
                  {t('carpet_page.highlights.dry_title')}
                </h4>
                <p className="text-xs text-gray-500 font-medium mt-1">
                  {t('carpet_page.highlights.dry_sub')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 justify-center md:justify-start border-t md:border-t-0 border-gray-100 pt-6 md:pt-0">
              ...
              <div>
                <h4 className="font-extrabold text-primary text-base leading-tight">
                  {t('carpet_page.highlights.eco_title')}
                </h4>
                <p className="text-xs text-gray-500 font-medium mt-1">
                  {t('carpet_page.highlights.eco_sub')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto mb-16">
            <h3 className="text-xl md:text-2xl font-extrabold text-primary text-center mb-8">
              {t('carpet_page.included_title')}
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 px-4">
              {(t('carpet_page.included_items', { returnObjects: true }) as string[]).map((item, index) => (
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-1 mb-6">
                    <span className="text-2xl md:text-3xl font-extrabold text-primary">{t(sz.priceKey)}</span>
                  </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center mb-16">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('carpet_page.faq_section_title')}
            </h2>
            <p className="text-gray-500">
              {t('carpet_page.faq_section_desc')}
            </p>
          </div>
            ...
            {faqs.map((faq, idx) => (
              ...
              >
                <button
                  ...
                  className="w-full text-left p-6 flex justify-between items-center gap-4 hover:bg-app-bg transition-colors cursor-pointer"
                >
                  <span className="font-bold text-primary text-base md:text-lg">
                    {t(`carpet_page.faqs.q${faq}`)}
                  </span>
      ...
      <section className="py-16 bg-primary text-white">
        <div className="container mx-auto px-4 text-center max-w-3xl">
          <h2 className="text-2xl md:text-4xl font-extrabold mb-6">Experience Premium Clean Carpets Today</h2>
          <p className="text-white/80 mb-8 max-w-xl mx-auto">
            Book our specialized German Kärcher® spray-extraction team today. Safe, deep-penetrating sanitization for a healthier, spotless environment.
          </p>
          <button 
            onClick={() => handleBookClick('Wall-to-Wall Office Carpet')}
            className="bg-action text-primary font-bold py-4.5 px-8 rounded-full text-lg flex items-center justify-center gap-2.5 mx-auto hover:scale-105 transition-transform cursor-pointer"
          >
            <MessageCircle size={22} />
            <span>Book On-Site Carpet Clean</span>
          </button>
        </div>
      </section>
    </div>
