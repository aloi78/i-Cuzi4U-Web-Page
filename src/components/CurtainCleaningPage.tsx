...
interface CurtainCleaningPageProps {
  onBack: () => void;
  onWhatsApp: (msg: string) => void;
}

export default function CurtainCleaningPage({ onBack, onWhatsApp }: CurtainCleaningPageProps) {
  const { t, i18n } = useTranslation();
  const [activeFaq, setActiveFaq] = useState<number | null>(null);

  useEffect(() => {
    // Dynamic SEO updates specifically for the Curtain Page
    const titles: Record<string, string> = {
      en: "On-Site Steam Curtain Cleaning & Sanitization Klang Valley | i-Cuzi4U",
      ms: "Servis Cuci Langsir Stim Atas Rel Lembah Klang | i-Cuzi4U",
      zh: "巴生谷免拆卸上门高温蒸汽窗帘除尘消毒服务 | i-Cuzi4U"
    };
    
    const descriptions: Record<string, string> = {
      en: "Professional on-site curtain steam cleaning service in Klang Valley. Deep dust mite extraction & 100°C steam sanitization without dismantling your drapes. Eco-friendly solutions.",
      ms: "Servis pembersihan stim langsir atas rel profesional di Lembah Klang. Basmi hama habuk & sanitasi stim 100°C tanpa cabut langsir. Larutan mesra alam.",
      zh: "巴生谷专业免拆卸上门窗帘高温蒸汽深度清洗除螨。采用德国 Kärcher® 抽洗与干蒸汽技术，不伤织物不缩水，安全环保、极速风干。"
    };
  ...
  const sizes = [
    {
      id: 'day',
      titleKey: 'curtain_page.sizes.day_title',
      priceKey: 'curtain_page.sizes.day_price',
      featuresKey: 'curtain_page.sizes.day_features',
      msg: 'Day Curtain'
    },
    {
      id: 'night',
      titleKey: 'curtain_page.sizes.night_title',
      priceKey: 'curtain_page.sizes.night_price',
      featuresKey: 'curtain_page.sizes.night_features',
      msg: 'Night / Blackout Curtain',
      featured: true
    },
    {
      id: 'double',
      titleKey: 'curtain_page.sizes.double_title',
      priceKey: 'curtain_page.sizes.double_price',
      featuresKey: 'curtain_page.sizes.double_features',
      msg: 'Double-Layer (Day + Night Pair)'
    }
  ];
  ...
  const handleBookClick = (sizeName: string) => {
    const baseMsg = t('curtain_page.cta_msg');
    ...
    <div className="bg-app-bg min-h-screen">
      ...
      <div className="bg-white border-b border-gray-100 py-4 sticky top-[72px] z-30 shadow-sm">
        <div className="container mx-auto px-4 flex items-center justify-between">
          ...
          >
            <ArrowLeft size={18} />
            <span>{t('curtain_page.back_btn')}</span>
          </button>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-10 max-w-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-sm text-gray-700">100% On-Site (No Removal)</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle2 size={20} className="text-action" />
                  ...
                  <div className="flex items-center gap-2">
                    <CheckCircle2 size={20} className="text-action" />
                    <span className="font-semibold text-sm text-gray-700">100°C Steam Sterilization</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle2 size={20} className="text-action" />
                    <span className="font-semibold text-sm text-gray-700">Quick-Dry (1-2 Hours)</span>
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row gap-4">
                  <button 
                    onClick={() => handleBookClick('Night / Blackout Curtain')}
                    className="btn-primary text-lg w-full sm:w-auto"
                  >
                    <MessageCircle size={22} />
                    <span>{t('curtain_page.cta_book')}</span>
                  </button>
                  <a 
                    href="#pricing"
            ...
            <div className="lg:col-span-5">
              ...
              >
                <img 
                  src="https://lh3.googleusercontent.com/d/1fEW3tYJj42TKajnCsz3N12HT0-wfu65J"
                  alt="Professional on-site steam curtain cleaning service - i-Cuzi4U"
                ...
                <div className="absolute bottom-6 left-6 bg-white/90 backdrop-blur-md px-5 py-3.5 rounded-2xl shadow-lg border border-white/20">
                  ...
                  <p className="text-lg font-extrabold text-primary">Kärcher® Steam Sanitization</p>
                </div>
              </motion.div>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('curtain_page.why_section_title')}
            </h2>
            <p className="text-gray-600 text-base md:text-lg">
              {t('curtain_page.why_section_desc')}
            </p>
          </div>
            ...
            {problems.map((prob, idx) => (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-primary mb-3">
                  {t(`curtain_page.problems.${prob.id}_title`)}
                </h3>
                <p className="text-gray-500 text-sm leading-relaxed">
                  {t(`curtain_page.problems.${prob.id}_desc`)}
                </p>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('curtain_page.process_section_title')}
            </h2>
            <p className="text-gray-600 text-base md:text-lg">
              {t('curtain_page.process_section_desc')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {stages.map((stage, idx) => (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-primary mb-3">
                  {t(`curtain_page.stages.${stage}_title`)}
                </h3>
                <p className="text-gray-600 text-sm leading-relaxed">
                  {t(`curtain_page.stages.${stage}_desc`)}
                </p>
              </motion.div>
            ))}
          </div>
      ...
      <section id="pricing" className="py-20 bg-app-bg border-y border-gray-100">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-12">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('curtain_page.pricing_section_title')}
            </h2>
            <p className="text-gray-600 text-base md:text-lg mb-4">
              {t('curtain_page.pricing_section_desc')}
            </p>
            {/* Shop Wash Coming Soon Notice Badge */}
          ...
          <div className="bg-white border border-gray-100 rounded-[2rem] p-6 md:p-8 max-w-4xl mx-auto mb-12 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex items-center gap-4 justify-center md:justify-start">
              ...
              <div>
                <h4 className="font-extrabold text-primary text-base leading-tight">
                  {t('curtain_page.highlights.effective_title')}
                </h4>
                <p className="text-xs text-gray-500 font-medium mt-1">
                  {t('curtain_page.highlights.effective_sub')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 justify-center md:justify-start border-t md:border-t-0 md:border-x border-gray-100 pt-6 md:pt-0 md:px-8">
              ...
              <div>
                <h4 className="font-extrabold text-primary text-base leading-tight">
                  {t('curtain_page.highlights.dry_title')}
                </h4>
                <p className="text-xs text-gray-500 font-medium mt-1">
                  {t('curtain_page.highlights.dry_sub')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 justify-center md:justify-start border-t md:border-t-0 border-gray-100 pt-6 md:pt-0">
              ...
              <div>
                <h4 className="font-extrabold text-primary text-base leading-tight">
                  {t('curtain_page.highlights.eco_title')}
                </h4>
                <p className="text-xs text-gray-500 font-medium mt-1">
                  {t('curtain_page.highlights.eco_sub')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto mb-16">
            <h3 className="text-xl md:text-2xl font-extrabold text-primary text-center mb-8">
              {t('curtain_page.included_title')}
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 px-4">
              {(t('curtain_page.included_items', { returnObjects: true }) as string[]).map((item, index) => (
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center mb-16">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('curtain_page.faq_section_title')}
            </h2>
            <p className="text-gray-500">
              {t('curtain_page.faq_section_desc')}
            </p>
          </div>
      ...
      <section className="py-16 bg-primary text-white">
        <div className="container mx-auto px-4 text-center max-w-3xl">
          <h2 className="text-2xl md:text-4xl font-extrabold mb-6">Enjoy Sanitized, Dust-Free Curtains Today</h2>
          <p className="text-white/80 mb-8 max-w-xl mx-auto">
            Book our specialized on-rail steam sanitization team today. No dismantle, no hassle, and absolutely zero fabric shrinkage risk.
          </p>
          <button 
            onClick={() => handleBookClick('Double-Layer (Day + Night Pair)')}
          ...
          >
            <MessageCircle size={22} />
            <span>Book On-Site Curtain Clean</span>
          </button>
        </div>
      </section>
    </div>
