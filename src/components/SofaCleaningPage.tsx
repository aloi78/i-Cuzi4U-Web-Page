...
interface SofaCleaningPageProps {
  onBack: () => void;
  onWhatsApp: (msg: string) => void;
}

export default function SofaCleaningPage({ onBack, onWhatsApp }: SofaCleaningPageProps) {
  const { t, i18n } = useTranslation();
  const [activeFaq, setActiveFaq] = useState<number | null>(null);

  useEffect(() => {
    // Dynamic SEO updates specifically for the Sofa Page
    const titles: Record<string, string> = {
      en: "Deep Sofa Cleaning Service Selangor & Couch Extraction PJ | i-Cuzi4U",
      ms: "Servis Cuci Sofa Selangor KL & Pengekstrakan Kusyen PJ | i-Cuzi4U",
      zh: "巴生谷专业沙发清洗与雪兰莪布艺及真皮沙发去污 | i-Cuzi4U"
    };
    
    const descriptions: Record<string, string> = {
      en: "Professional deep sofa cleaning service in Selangor & KL. Specialized fabric and leather extraction using eco-friendly German Kärcher® technology.",
      ms: "Servis cuci sofa profesional di Selangor, KL, dan PJ. Basmi kotoran, hama, minyak, dan bau pada kusyen fabrik atau kulit dengan teknologi Kärcher® Jerman.",
      zh: "雪兰莪及吉隆坡专业沙发清洗。针对布艺和真皮沙发，利用德国 Kärcher® 喷抽设备、生物活性酶深层去渍除螨、异味中和分解，温和无害、环保配方。"
    };
  ...
  const problems = [
    ...
    { id: 'pet', icon: <Heart className="text-red-400" size={28} /> }
  ];
  ...
  const sizes = [
    {
      id: 'single',
      titleKey: 'sofa_page.sizes.single_title',
      priceKey: 'sofa_page.sizes.single_price',
      featuresKey: 'sofa_page.sizes.single_features',
      msg: '1 Seater / Armchair'
    },
    {
      id: 'queen',
  ...
  const handleBookClick = (sizeName: string) => {
    const baseMsg = t('sofa_page.cta_msg');
    onWhatsApp(`${baseMsg} ${sizeName}`);
  };

  return (
    <div className="bg-app-bg min-h-screen" id="sofa-cleaning-page">
      {/* Breadcrumb Header */}
      <div className="bg-white border-b border-gray-100 py-4 sticky top-[72px] z-30 shadow-sm">
        <div className="container mx-auto px-4 flex items-center justify-between">
          ...
          >
            <ArrowLeft size={18} />
            <span>{t('sofa_page.back_btn')}</span>
          </button>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-7">
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-10 max-w-lg">
                  ...
                  <div className="flex items-center gap-2">
                    <CheckCircle2 size={20} className="text-action" />
                    <span className="font-semibold text-sm text-gray-700">Leather & Fabric Experts</span>
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row gap-4">
                  <button 
                    onClick={() => handleBookClick('2 to 3 Seater Sofa')}
                    className="btn-primary text-lg w-full sm:w-auto animate-pulse-subtle"
                  >
                    <MessageCircle size={22} />
                    <span>{t('sofa_page.cta_book')}</span>
                  </button>
                  <a 
                    href="#pricing"
            ...
            <div className="lg:col-span-5">
              ...
              >
                <img 
                  src="https://lh3.googleusercontent.com/d/1mihf8YH6_ZrypGBGnlMswhJCvEzY4Qcb"
                  alt="Professional fabric and leather sofa cleaning PJ Selangor KL - i-Cuzi4U"
      ...
      {/* Why Section */}
      <section className="py-20 bg-app-bg" id="sofa-why-section">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('sofa_page.why_section_title')}
            </h2>
            <p className="text-gray-600 text-base md:text-lg">
              {t('sofa_page.why_section_desc')}
            </p>
          </div>
            ...
            {problems.map((prob, idx) => (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-primary mb-3">
                  {t(`sofa_page.problems.${prob.id}_title`)}
                </h3>
                <p className="text-gray-500 text-sm leading-relaxed">
                  {t(`sofa_page.problems.${prob.id}_desc`)}
                </p>
              </motion.div>
            ))}
          </div>
      ...
      {/* Process Section */}
      <section className="py-20 bg-white" id="sofa-process-section">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('sofa_page.process_section_title')}
            </h2>
            <p className="text-gray-600 text-base md:text-lg">
              {t('sofa_page.process_section_desc')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {stages.map((stage, idx) => (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-primary mb-3">
                  {t(`sofa_page.stages.${stage}_title`)}
                </h3>
                <p className="text-gray-600 text-sm leading-relaxed">
                  {t(`sofa_page.stages.${stage}_desc`)}
                </p>
              </motion.div>
            ))}
          </div>
      ...
      <section id="pricing" className="py-20 bg-app-bg border-y border-gray-100">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto mb-12">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('sofa_page.pricing_section_title')}
            </h2>
            <p className="text-gray-600 text-base md:text-lg">
              {t('sofa_page.pricing_section_desc')}
            </p>
          </div>
          ...
          <div className="bg-white border border-gray-100 rounded-[2rem] p-6 md:p-8 max-w-4xl mx-auto mb-12 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex items-center gap-4 justify-center md:justify-start">
              ...
              <div>
                <h4 className="font-extrabold text-primary text-base leading-tight">
                  {t('sofa_page.highlights.effective_title')}
                </h4>
                <p className="text-xs text-gray-500 font-medium mt-1">
                  {t('sofa_page.highlights.effective_sub')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 justify-center md:justify-start border-t md:border-t-0 md:border-x border-gray-100 pt-6 md:pt-0 md:px-8">
              ...
              <div>
                <h4 className="font-extrabold text-primary text-base leading-tight">
                  {t('sofa_page.highlights.dry_title')}
                </h4>
                <p className="text-xs text-gray-500 font-medium mt-1">
                  {t('sofa_page.highlights.dry_sub')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 justify-center md:justify-start border-t md:border-t-0 border-gray-100 pt-6 md:pt-0">
              ...
              <div>
                <h4 className="font-extrabold text-primary text-base leading-tight">
                  {t('sofa_page.highlights.eco_title')}
                </h4>
                <p className="text-xs text-gray-500 font-medium mt-1">
                  {t('sofa_page.highlights.eco_sub')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto mb-16">
            <h3 className="text-xl md:text-2xl font-extrabold text-primary text-center mb-8">
              {t('sofa_page.included_title')}
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 px-4">
              {(t('sofa_page.included_items', { returnObjects: true }) as string[]).map((item, index) => (
      ...
      {/* FAQs Section */}
      <section className="py-20 bg-white" id="sofa-faq-section">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center mb-16">
            <h2 className="text-2xl md:text-4xl font-extrabold text-primary mb-4">
              {t('sofa_page.faq_section_title')}
            </h2>
            <p className="text-gray-500">
              {t('sofa_page.faq_section_desc')}
            </p>
          </div>
      ...
      {/* Mini CTA */}
      <section className="py-16 bg-primary text-white" id="sofa-mini-cta">
        <div className="container mx-auto px-4 text-center max-w-3xl">
          <h2 className="text-2xl md:text-4xl font-extrabold mb-6">Enjoy a Stain-Free, Sanitized Sofa Tonight</h2>
          <p className="text-white/80 mb-8 max-w-xl mx-auto">
            Book our professional deep extraction and disinfection team today. Eco-friendly, deep-sanitizing, and highly effective.
          </p>
          <button 
            onClick={() => handleBookClick('2 to 3 Seater Sofa')}
          ...
          >
            <MessageCircle size={22} />
            <span>Book 3-Seater Sofa Clean</span>
          </button>
        </div>
      </section>
    </div>
